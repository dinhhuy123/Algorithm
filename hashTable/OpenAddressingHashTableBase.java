...
public abstract class OpenAddressingHashTableBase<K, V> implements HashTableADT<K, V> {
    ...
    public OpenAddressingHashTableBase(double loadFactor, int capacity) {
		if (capacity < 0) throw new IllegalArgumentException("Error in capacity");
		if (loadFactor <= 0 || Double.isNaN(loadFactor) || Double.isInfinite(loadFactor)) {
			throw new IllegalArgumentException("Error in load factor!");
		}
	...
	public V insert(K key, V value) {
		...
		for (int i = offset, x = 1, firstTomstoneIndex = -1; ; i = hashCodeToIndex(offset + probe(x++))) {
			if (keys[i] == TOMBSTONE) {
				if (firstTomstoneIndex == -1) {
					firstTomstoneIndex = i;
				}
			} else if (keys[i] != null) {
				if (keys[i].equals(key)) {
					...
					if (firstTomstoneIndex == -1) {
						...
						keys[firstTomstoneIndex] = key;
						values[firstTomstoneIndex] = value;
						keys[i] = TOMBSTONE;
						values[i] = null;
					}
					return oldValue;
		...
		for (int i = offset, x = 1, firstTomstoneIndex = -1; ; i = hashCodeToIndex(offset + probe(x++))) {
			if (keys[i] == TOMBSTONE) {
				...
				if (keys.equals(key)) {
					if (firstTomstoneIndex != -1) {
						keys[firstTomstoneIndex] = key;
						values[firstTomstoneIndex] = values[i];
						keys[i] = TOMBSTONE;
						values[i] = null;
					}
					return true;
